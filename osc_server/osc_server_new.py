...
class MuseServer(ServerThread):
    ...
    def eeg_callback(self, path, args):
        ...
        if np.shape(self.raw)[0] == 22:
            ...
            # print v_sec
            vals_list = v_sec[0].tolist()
            vals_json = json.dumps(vals_list)
            print np.shape(vals_list)
            print vals_json
            # print "sending 'Muse channel TP9 data from Python'..."
            # self.ws.send(vals_json)
            # result = self.ws.recv()
            # print "Receiving..."
            # print "Received: %s'" % result
            self.sock.sendto(vals_json, (UDP_IP, UDP_PORT))
            # for i in xrange(4):
            #     # print "line 54"
            #     cv = v_sec[i]
                # alpha = mne.filter.band_pass_filter(x=cv, Fs=110, Fp1=7.5, Fp2=14)
                # beta = mne.filter.band_pass_filter(x=cv, Fs=110, Fp1=14, Fp2=32)
                # gamma = mne.filter.band_pass_filter(x=cv, Fs=110, Fp1=32, Fp2=64)
